...
until [ "$validMenuInput" = true ]; do
    ...
    for (( i=1; i<=$fieldMenuSelection; i++)); do
        ...
        if [ $searchColumnMenuSelection = 1 ]; then
            validMenuInput=false # Set vaildMenuInput back to false
            until [ "$validMenuInput" = true ]; do
        ...
        elif [[ $searchColumnMenuSelection = 6 || $searchColumnMenuSelection = 7 ]]; then
    ...
    for ((j=0; j<=${#searchDetailsArray[@]}-1; j++)); do
        if [ ${searchDetailsArray[field$(($j))]} = "PROTOCOL" ]; then
            # Search for records using PROTOCOL
            grep -i "${searchDetailsArray[search$(($j))]}" $accessLogSelected
        fi
    done
